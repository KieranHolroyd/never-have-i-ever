...
<div class="text-center py-12">
	...
	<div class="mb-8">
		<div
			class="inline-flex items-center justify-center w-20 h-20 bg-yellow-500/20 rounded-full mb-6"
		>
		...
		<p class="text-slate-400 max-w-md mx-auto">What an amazing game! Here are the final results.</p>
	</div>
	...
	<div class="max-w-2xl mx-auto">
		...
		<div class="space-y-3">
			{#each sortedPlayers as player, i (player.id)}
				<div
					class="relative bg-slate-700/50 rounded-lg p-4 transition-all duration-200
					{i === 0
				...
				>
					<!-- Rank Badge -->
					<div
						class="absolute -left-4 top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold
						{i === 0
					...
					<div class="flex items-center justify-between ml-8">
						<div class="flex items-center gap-4">
							<!-- Player Avatar -->
							<div
								class="w-12 h-12 rounded-full bg-gradient-to-br from-slate-600 to-slate-800 flex items-center justify-center"
							>
							...
							<div class="text-left">
								<div class="flex items-center gap-2">
									...
									{#if player.id === currentPlayerId}
										<span
											class="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-full text-xs font-medium"
											>(You)</span
										>
									{/if}
								</div>
