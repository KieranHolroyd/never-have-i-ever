...
<div class="mb-6">
	<div class="flex items-center justify-between mb-4">
		<div class="flex items-center gap-3">
			...
			<div class="flex items-center gap-2 px-3 py-1 bg-purple-500/20 rounded-full text-sm text-purple-400 font-medium">
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
		{#each submissions as submission, index (submission.playerId)}
			...
			>
				...
				<!-- Submission Badge -->
				<div class="absolute -top-3 -left-3 w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-lg">
					{index + 1}
				</div>
				...
				<div class="mt-4 pt-3 border-t border-gray-200 flex items-center justify-between">
					<div class="flex items-center gap-2">
						<div class="w-6 h-6 bg-slate-600 rounded-full flex items-center justify-center">
							<span class="text-xs font-bold text-white">{submission.playerName.charAt(0).toUpperCase()}</span>
						</div>
						<span class="text-sm font-medium text-gray-700">{submission.playerName}</span>
					</div>

					<!-- Vote Button -->
					<div class="flex items-center gap-2 px-3 py-1 bg-purple-500 text-white rounded-full text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
				...
				<!-- Hover Effect -->
				<div class="absolute inset-0 bg-gradient-to-r from-purple-400/0 to-purple-400/5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg"></div>
			</button>
		{/each}
	</div>
