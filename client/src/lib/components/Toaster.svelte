...
<div class="fixed z-50 top-4 right-4 w-[calc(100vw-2rem)] max-w-sm space-y-3 pointer-events-none">
	{#each items as t (t.id)}
		<div
			class="pointer-events-auto border rounded-2xl shadow-lg overflow-hidden text-white/90 bg-gradient-to-br {containerClasses(
				t.type
			)} ring-1"
		...
		>
			<div class="relative">
				<div
					class="absolute inset-y-0 left-0 w-px bg-gradient-to-b {accentBarClasses(
						t.type
					)} opacity-60"
				></div>
				<div class="px-4 py-3 pl-5 flex items-center gap-3">
					<div
						class="h-6 w-6 shrink-0 rounded-full grid place-items-center ring-1 {iconWrapClasses(
							t.type
						)}"
					>
						{#if t.type === 'success'}
